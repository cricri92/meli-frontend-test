...
function formatItemResult(result) {
	return {
		...
		picture: result.pictures && result.pictures.length > 0 ? result.pictures[0].url : result.thumbnail,
...
const itemsMiddleware = {
	formatItemsResults(searchResults, chunkSize = 4) {
		...
		return {
			categories: categories && categories.values ? categories.values.map(category => ({ category_id: category.id, category_name: category.name })) : null,
